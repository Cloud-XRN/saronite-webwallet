...
demonstrate how to provide Saronite related JSON REST service. For this purpose,
 a service called Saronite Webwallet was developed.


Saronite Webwallet is an open source implementation of backend of
https://mymonero.com/ adapted for Saronite. The frontend, which includes HTML, CSS, JavaScript, was adapted
from (and originally developed by) https://mymonero.com/.

Saronite Webwallet's backend is open sourced, free
...
to MyMonero. They include:
 ...
 - the wallets generated use 25 word mnemonics, fully compatible with official Saronite wallets
(13 word mnemonics generated by MyMonero work as usual though).
 ...
 - private tx key given to a user which can be used to prove sending SARONITE to a recipient.
...
The Saronite Webwallet consists of four components that need to be setup for it to work:
 ...
 - Saronite daemon - daemon must be running and fully sync, as this is
 where all transaction data is fetched from and used. Daemon also commits txs
 from the Saronite Webwallet into the Saronite network.
 - Backend - fully written in C++. It uses [restbed](https://github.com/Corvusoft/restbed/) to serve JSON REST to the frontend
 and [mysql++](http://www.tangentsoft.net/mysql++/) to interface the database. It also accesses Saronite blockchain and "talks"
 to Saronite daemon.
...
Saronite Webwallet is not as fast as MyMonero.
...
Below are example and basic instructions on how to setup up and run Saronite Webwallet on Ubuntu 16.04.
For other Linux operating systems, the instructions are analogical.


#### Saronite download and compilation

Download and compile recent Saronite into your home folder:

```bash
# first install Saronite dependencies
sudo apt update
...
#### Compilation of the Saronite Webwallet (don't run it yet)

Once Saronite was downloaded and compiled, we can download Saronite Webwallet and compile it.
...
work without database, setup frontend, and synced and running Saronite blockchain.

```bash
# need mysql++ libraries
sudo apt install libmysql++-dev

# go to home folder if still in ~/.bitsaronite
cd ~

git clone https://github.com/saronite/saronite-webwallet.git

cd saronite-webwallet

mkdir build && cd build

cmake ..

# altearnatively can use cmake -DSARONITE_DIR=/path/to/saronite_folder ..
# if Saronite is not in ~/.bitsaronite
...
Download `db.sql` provided and setup the `saronite_wallet` database. `db.sql` script will
drop current `saronite_wallet` if exist. So don't run it, if you have already some important information
in the `saronite_wallet` database.
...
```bash
sudo mkdir /var/www/html/saronite-webwallet
sudo cp -rvf ../html/* /var/www/html/saronite-webwallet/
```

Setup document root in `lighttpd.conf` into saronite-webwallet folder
...
```bash
server.document-root    = "/var/www/html/saronite-webwallet"
```
...
#### Run Saronite Webwallet

Command line options

```bash
./saronite-webwallet -h
...
Before running `saronite-webwallet`:
 ...
 - make sure Saronite daemon is running and fully sync. If using testnet or stagenet networks, use Saronite daemon
...
```bash
./saronite-webwallet -t
```

To start for stagenet:
```bash
./saronite-webwallet -s
```

To start for stagenet with non-default location of `config.json` file:

```bash
./saronite-webwallet -s -c /path/to/config.json
```


## Saronite Webwallet JSON REST API

Example JSON REST requests and their responses of [Saronite Webwallet](https://github.com/saronite/saronite-protocol-webwallet) are provided below. The long term goal
...
the Saronite Webwallet API does not fully conform to that.

#### get_version

Get version of the Saronite Webwallet, its API and Saronite.
...
```json
{
  ...
  "saronite_version_full": "0.2.0-master"
}
```
...
Login an existing or new user into Saronite Webwallet
...
Get random outputs from the Saronite daemon to be used as ring members
...
Submit raw tx in hex generated by the frontend to be relayed to the Saronite network.
